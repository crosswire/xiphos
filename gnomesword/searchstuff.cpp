...
//-------------------------------------------------------------------------------------------
void
SearchWindow::searchSWORD(GtkWidget *searchFrm)  //-- search Bible text or commentaries
{
	ModMap::iterator it;
	...
	if (searchMod) /* must have a good module - not null */
	{
		...
		for (ListKey &searchResults = searchMod->Search(entryText, searchType, searchParams, currentScope); !searchResults.Error(); searchResults++)
		{
...
SearchWindow::create()
{
  ...
  btnSearchOK = (GtkWidget *)g_list_last (GNOME_DIALOG (dlgSearch)->buttons)->data;
