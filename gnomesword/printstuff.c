 ...
 **/
void
file_print(GtkWidget * widget, gchar * current_filename,
	   gint file_printpreview)
{
	...
	if (file_printpreview) {
		dialog =
		    gnome_print_dialog_new((const char *) "Print Document",
		...
		gnome_print_dialog_construct_range_page(
							...
							1, pji->pages, "A",
							_("Pages")
							/* Translators: As in [Range] Pages from:[x]  to */
							);
		...
		printer =
		    gnome_print_dialog_get_printer(GNOME_PRINT_DIALOG
						   (dialog));

		pji->print_first = 0;
		pji->print_last = 0;
		pji->range =
		    gnome_print_dialog_get_range_page(GNOME_PRINT_DIALOG
						      (dialog),
						      &pji->print_first,
						      &pji->print_last);
	...
	if (pji->preview) {
		...
		title =
		    g_strdup_printf(_("GnomeSword (%s): Print Preview"),
				    pji->filename);
		preview =
		    gnome_print_master_preview_new_with_orientation(pji->
								    master,
								    title,
								    !pji->
								    orientation);
		...
		gtk_signal_connect(GTK_OBJECT(preview), "destroy",
				   GTK_SIGNAL_FUNC(preview_destroy_cb),
				   pji);
		gtk_widget_show(GTK_WIDGET(preview));
	} else {
		if (printer)
			gnome_print_master_set_printer(pji->master,
						       printer);
...
print_document(gchar * doc, PrintJobInfo * pji, GnomePrinter * printer)
{
	...
	for (current_page = 1; current_page <= pji->pages; current_page++) {
		if (pji->range != GNOME_PRINT_RANGE_ALL)
			pji->print_this_page =
			    (current_page >= pji->print_first
			     && current_page <=
			     pji->print_last) ? TRUE : FALSE;
		else
		...
		if (pji->print_this_page) {
			...
			pagenumbertext =
			    g_strdup_printf("%d", current_page);
		...
		for (current_line++; current_line <= pji->total_lines;
		     current_line++) {
...
static void print_line(PrintJobInfo * pji, int line)
{
	...
	while (pji->buffer[pji->file_offset] != '\n'
		...
		if (pji->buffer[pji->file_offset] == '\t') {
			...
			chars_in_this_line +=
			    pji->tab_size -
			    ((chars_in_this_line - 1) % pji->tab_size) - 1;

			if (chars_in_this_line > pji->chars_per_line + 1)
				chars_in_this_line =
				    pji->chars_per_line + 1;
		...
		if (chars_in_this_line == pji->chars_per_line + 1) {
			if (!pji->wrapping) {
				...
				while (pji->buffer[pji->file_offset] != ' '
				       ...
				       '\t'
				       && pji->file_offset >
				       temp - pji->chars_per_line - 1) {
					pji->file_offset--;
				}

				if (dump_info)
					g_print
					    ("file offset got backed up [%i] times\n",
				...
				if (dump_info) {
					g_print("Breaking temp at : %i\n",
						...
						1);
					g_print
					    ("Chars_in_this_line %i File Offset %i Temp %i\n",
					     chars_in_this_line,
					     pji->file_offset, temp);
				}
				pji->temp[chars_in_this_line +
					  pji->file_offset - temp - 1] =
				    (guchar) '\0';
...
static void print_ps_line(PrintJobInfo * pji, gint line, gint first_line)
{
	...
	y = pji->page_height - pji->margin_top - pji->header_height -
	    (pji->font_char_height *
	     ((pji->current_line % pji->lines_per_page) + 1));
...
//      if (settings->print_lines > 0)  pji->margin_left += pji->margin_numbers;
	...
	pji->printable_width = pji->page_width - pji->margin_left - pji->margin_right;	//-((settings->print_lines>0)?pji->margin_numbers:0)
	pji->printable_height = pji->page_height -
	    pji->margin_top - pji->margin_bottom;
	...
	pji->chars_per_line =
	    (gint) (pji->printable_width / pji->font_char_width);
	pji->lines_per_page =
	    (guint) (pji->printable_height -
		     pji->header_height) / (guint) pji->font_char_height -
	    1;
	pji->tab_size = 8;
	pji->total_lines = print_determine_lines(pji, FALSE);
	pji->total_lines_real = print_determine_lines(pji, TRUE);
	pji->pages =
	    ((int) (pji->total_lines_real - 1) / pji->lines_per_page) + 1;
	pji->file_offset = 0;
	pji->current_line = 0;
}
...
static guint print_determine_lines(PrintJobInfo * pji, int real)
{
	...
	if (dump_info_basic) {
		if (real)
			g_print
			    ("Determining lines in REAL mode. Lines Per page =%i\n",
			     lines_per_page);
		else
			g_print
			    ("Determining lines in WRAPPING mode. Lines Per page =%i\n",
			     lines_per_page);
	}
	...
	for (i = 0; i < buffer_size; i++) {
		...
		if (buffer[i] == '\t') {
			...
			chars_in_this_line +=
			    tab_size -
			    ((chars_in_this_line - 1) % tab_size) - 1;
		...
		if (chars_in_this_line == chars_per_line + 1 && real) {
			...
			if (dump_info && i != temp_i)
				g_print
				    ("We removed %i trailing spaces/tabs",
				     i - temp_i);
	...
	if (dump_info_basic && lines != temp_i) {
		g_print
		    ("We removed %i line(s) because they contained no text\n",
		     temp_i - lines);
	}
