...
void backend_newDisplayDL(GtkWidget * html, char *bookname, SETTINGS * s)
{
    GList *tmp = NULL;
    BE_DL *d;
...
void backend_setupDL(SETTINGS * s)
{
    ...
    be_dl_list = NULL;
    for (it = swmgrDL->Modules.begin(); it != swmgrDL->Modules.end(); it++) {
	if (!strcmp((*it).second->Type(), "Lexicons / Dictionaries")) {
	    BE_DL *be_dl = new BE_DL;
	    be_dl->mod = (*it).second;
	    be_dl->num = count;
	    be_dl_list = g_list_append(be_dl_list, (BE_DL *) be_dl);
	    ++count;
	}
    }
}
