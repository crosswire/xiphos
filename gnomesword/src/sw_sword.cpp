...
gboolean noteModified = false,	/* set to true is personal note has changed */
    ...
    showtexts = true,		/* show text, commentary and dictionary panes by default */
    showcomms = true, showdicts = true;
...
void initSWORD(SETTINGS * s)
{
	...
	commDisplay =
	    new
	    GtkHTMLEntryDisp(lookup_widget(s->app, "htmlCommentaries"),
			     s);
	comp1Display = new InterlinearDisp(s->htmlInterlinear, s);
	FPNDisplay = new GtkHTMLEntryDisp(htmlComments, s);
	dictDisplay =
	    new GtkHTMLEntryDisp(lookup_widget(s->app, "htmlDict"), s);
	compages = 0;
	dictpages = 0;
	textpages = 0;
	bookpages = 0;
	...
	if (settings->modifiedSP) {	//-- if study pad file has changed since last save  
		...
		case 0:
			if (settings->studypadfilename)
				save_file_program_end(settings->htmlSP,
						      settings->
						      studypadfilename);
			break;
		default:
...
{
	GtkWidget *texts, *hpaned1;
	texts = lookup_widget(settings->app, "vbox32");
	hpaned1 = lookup_widget(settings->app, "hpaned1");
...
{
	GtkWidget *comms, *vpaned1, *hpaned1;
	comms = lookup_widget(settings->app, "notebook3");
	hpaned1 = lookup_widget(settings->app, "hpaned1");
	settings->showcomms = choice;
	if (choice == FALSE) {
		gtk_widget_hide(comms);
	} else {
		gtk_widget_show(comms);
	}
	setbiblecommlayoutSWORD();
}
...
{
	GtkWidget *dicts, *vpaned1;
	dicts = lookup_widget(settings->app, "hbox8");
	vpaned1 = lookup_widget(settings->app, "vpaned1");
	settings->showdicts = choice;
	if (choice == FALSE) {
		gtk_widget_hide(dicts);
	} else {
		gtk_widget_show(dicts);
	}
	setbiblecommlayoutSWORD();
}
...
void setbiblecommlayoutSWORD()
{
	if ((settings->showtexts == FALSE)
		...
		e_paned_set_position(E_PANED
				     (lookup_widget
				      (settings->app, "vpaned1")), 0);
	} else if (settings->showdicts == FALSE) {
		e_paned_set_position(E_PANED
				     (lookup_widget
				      (settings->app, "vpaned1")),
				     settings->gs_hight);
	} else {
		e_paned_set_position(E_PANED
				     (lookup_widget
...
void showmoduleinfoSWORD(char *modName, gboolean isGBS)	//--  show module information in an about dialog
{
	...
	*newbuf, discription[500];
	...
	if (strcmp(bufabout, "oops")) {
		...
		displayHTML(text, HTML_START "<body>",
			    strlen(HTML_START "<body>"));
