...
void gui_setPageandKey_DL(gint page_num, gchar * key)
{
    ...
    gtk_notebook_set_page(GTK_NOTEBOOK(settings->notebookDL), page_num);
...
static void on_entryDictLookup_changed(GtkEditable * editable, DL_DATA * d)
{
    gint count;
    gchar *key, *text;
    GList *dictkeys = NULL;
			   ...
			   gint column, GdkEvent * event, DL_DATA * d)
{
    ...
    gtk_clist_get_text(GTK_CLIST(GTK_WIDGET(clist)), row, column, &text);
...
void on_showtabs_activate(GtkMenuItem * menuitem, SETTINGS * s)
{
    ...
    gtk_notebook_set_show_tabs(GTK_NOTEBOOK(s->notebookDL), s->dict_tabs);
}
...
GtkWidget *create_pmDL(DL_DATA * dl, GList * mods)
{
    ...
    dl->showtabs = gtk_check_menu_item_new_with_label(_("Show Tabs"));
	...
	gtk_signal_connect(GTK_OBJECT(item3), "activate",
			   GTK_SIGNAL_FUNC
			   (on_view_activate), GINT_TO_POINTER(i));
				     ...
				     DL_DATA * dl, gint count)
{
    ...
    gtk_object_set_data_full(GTK_OBJECT(s->app), "hpaned7",
			     hpaned7, (GtkDestroyNotify) gtk_widget_unref);
    ...
    gtk_object_set_data_full(GTK_OBJECT(s->app), "vbox56",
			     vbox56, (GtkDestroyNotify) gtk_widget_unref);
    ...
    gtk_box_pack_start(GTK_BOX(vbox56), toolbarDLKey, FALSE, TRUE, 0);
	...
	gtk_toolbar_append_element(GTK_TOOLBAR(toolbarDLKey),
				   ...
				   _("Sync"), _("Load current key"), NULL,
    ...
    gtk_toolbar_append_widget(GTK_TOOLBAR(toolbarDLKey), dl->entry, NULL,
			      NULL);
    ...
    gtk_container_add(GTK_CONTAINER(scrolledwindowDictHTML), dl->html);
    ...
    gtk_signal_connect(GTK_OBJECT(dl->entry), "changed",
		       GTK_SIGNAL_FUNC(on_entryDictLookup_changed), dl);
    gtk_signal_connect(GTK_OBJECT(dl->clist), "select_row",
		       GTK_SIGNAL_FUNC(on_clistDictLex_select_row), dl);
