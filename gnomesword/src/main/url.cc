...
gint main_url_handler(const gchar * url, gboolean clicked)
{
	...
	if (strstr(url_clean->str, "sword://")   ||
	    strstr(url_clean->str, "book://")    ||
	    strstr(url_clean->str, "chapter://")) {
		int retval = sword_uri(url_clean->str, clicked);
	...
	if (strstr(url, "passagestudy.jsp") ||
		...
		m_url = new URL((const char*)url_clean->str);
