...
class GtkTextIterator { GtkText *text;
	guint pos_;
	...
	self & go_end() {
		pos_ = gtk_text_get_length(text);
		return *this;
	}
	self & go_cur() {
		pos_ = gtk_editable_get_position(GTK_EDITABLE(text));
		return *this;
	}
	guint pos() const {
		return pos_;
	} const char operator *() const {
		return GTK_TEXT_INDEX(text, pos_);
	} self & operator++ () {
		++pos_;
		return *this;
	}
	...
	friend bool operator == (const self & rhs, const self & lhs)
		...
		return rhs.pos_ - lhs.pos_;
	} friend self & operator - (self rhs, guint lhs) {
		return rhs -= lhs;
	} friend self & operator + (self rhs, guint lhs) {
		return rhs += lhs;
}};
...
class AspellCheck { GtkWidget *window;
...
//  string            file_name;
	 ...
	 save_cont(do_nothing), sc(myoptions), state(sc), do_savedicts(true),
		...
		if (myoptions.have("dict-host"))
			if (!myoptions.have("dict-port"))
				dc.setHost(myoptions.retrieve("dict-host").
					   c_str());
			else
				dc.setHost(myoptions.retrieve("dict-host").
					   c_str(),
					   myoptions.
					   retrieve_int("dict-port"));
	} ~AspellCheck() {
		if (do_savedicts)
			sc.save_all_wls();
	}
	...
	GtkWidget *win() {
	    ...
	    enum problem { no_errors, no_file, could_not_open, canceled,
		      exists };
	...
	void set_main_dlg(GtkWidget * w) {
		main_dlg = w;
	}
	// these members affect which buttons are activetor for the GtkText widgetUSA.
	    void edit_mode();
	void check_mode();
	void sug_mode();
	...
	// starts checking at the curser position activetor for the GtkText widgetUSA.
	    bool exit();
	friend void text_insert(GtkWidget *, const gchar *, gint, gint *,
				gpointer);
...
class AspellDlg { AspellCheck check;
      public:
	 GtkWidget * main_dlg;
	 AspellDlg() {
	} void draw();
...
void AspellDlg::draw()
{
	...
	gtk_object_set_data(GTK_OBJECT(main_dlg), "dialog_vbox4",
			    dialog_vbox4);

	check.draw();
	gtk_box_pack_start(GTK_BOX(dialog_vbox4), check.win(), TRUE, TRUE,
			   2);
...
void AspellCheck::draw()
{
	...
	case 0:
		text = studypad;	//-- check text in studypad window
		break;
	case 1:
		text = notes;	//-- check text in notes window
		break;
	default:
		text = studypad;	//-- check text in studypad window
		break;
...
//  gtk_paned_add2(mybox, paned2);
	...
	gtk_text_insert(GTK_TEXT(definition), 0, 0, 0,
			definition_msg.c_str(), definition_msg.size());
...
#undef add_button
	...
	gtk_table_attach_defaults(GTK_TABLE(table), auto_lookup_, 1, 2, 4,
				  5);
...
{
	if (whichtextwidget == 1)
		noteModified = true;	//-- notes have been changed
	if (bSpell) {
		changed = true;
		unhighlight_text();
...
void AspellCheck::check()
{
	...
	if (wrapped && (state.at_end() || mis_begin >= start_pos)) {
		...
		word =
		    gtk_editable_get_chars(GTK_EDITABLE(text), mis_begin,
					   mis_end);
		gtk_text_set_point(GTK_TEXT(text), mis_begin);
		gtk_text_forward_delete(GTK_TEXT(text),
					mis_end - mis_begin);

		gtk_text_insert(GTK_TEXT(text), 0, &misspelled_color, 0,
				word, -1);
...
void AspellCheck::change()
{
	...
	state.backup();
	gtk_text_insert(GTK_TEXT(text), 0, &text->style->black, 0, repl,
			-1);
...
GtkWidget *spellcheck(gint itext)
{
	...
	ConfigData::PossibleElementsEmul els =
	    myoptions.possible_elements();
	const KeyInfo *k;
	while (k = els.next(), k) {
		if (k->desc == 0)
			continue;
...
void options_act(poptContext con, poptCallbackReason,
		 const struct poptOption *opt, const char *arg, void *)
{
	if (!arg)
		arg = "";
	try {
