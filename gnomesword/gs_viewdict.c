 ...
 *
 */
void on_dlgShowDict_destroy(GtkObject * object, gpointer user_data)
{
	isrunningSD = FALSE;
	shutdownSDSWORD();
}
...
void on_btnSDClose_clicked(GtkButton * button, gpointer user_data)
{
...
GtkWidget *create_dlgShowDict(void)
{
	...
	gtk_object_set_data(GTK_OBJECT(dlgShowDict), "dlgShowDict",
			    dlgShowDict);
	gtk_container_set_border_width(GTK_CONTAINER(dlgShowDict), 4);
	gtk_window_set_title(GTK_WINDOW(dlgShowDict),
			     "GnomeSword - Show Dictionary");
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "toolbar27",
				 toolbar27,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "cbSDMods",
				 cbSDMods,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "ceSDMods",
				 ceSDMods,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "label126",
				 label126,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "entrySDLookup",
				 entrySDLookup,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	tmp_toolbar_icon =
	    gnome_stock_pixmap_widget(dlgShowDict,
				      GNOME_STOCK_PIXMAP_CLOSE);
	btnSDClose =
	    gtk_toolbar_append_element(GTK_TOOLBAR(toolbar27),
				       GTK_TOOLBAR_CHILD_BUTTON, NULL,
				       "Close", "Close Dictionary Dialog",
				       NULL, tmp_toolbar_icon, NULL, NULL);
	gtk_widget_ref(btnSDClose);
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "btnSDClose",
				 btnSDClose,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "frameShowDict",
				 frameShowDict,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "hpaned2",
				 hpaned2,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict),
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "label125",
				 label125,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_object_set_data_full(GTK_OBJECT(dlgShowDict), "textSDmodule",
				 textSDmodule,
				 (GtkDestroyNotify) gtk_widget_unref);
	...
	gtk_signal_connect(GTK_OBJECT(dlgShowDict), "destroy",
			   GTK_SIGNAL_FUNC(on_dlgShowDict_destroy), NULL);
	gtk_signal_connect(GTK_OBJECT(ceSDMods), "changed",
			   GTK_SIGNAL_FUNC(on_ceSDMods_changed), NULL);
	...
	gtk_signal_connect(GTK_OBJECT(btnSDClose), "clicked",
			   GTK_SIGNAL_FUNC(on_btnSDClose_clicked), NULL);
