...
COMM_DATA *getCOMM ( GList * cl )
{
        ...
        while ( tmp != NULL ) {
                ...
                if ( !strcmp
                     ( c->modName, settings->CommWindowModule ) ) {
                        break;
                }
                tmp = g_list_next ( tmp );
        }
...
/*
    ...
    */
        ...
        GTK_CHECK_MENU_ITEM ( c->showtabs )->active =
            settings->comm_tabs;
}
                          ...
                          SETTINGS * s )
{
        ...
        gtk_notebook_set_page ( GTK_NOTEBOOK ( s->notebookCOMM ),
                                page );
        s->commLastPage = page;
...
GtkWidget *create_pmCOMM ( COMM_DATA * c )
{
        ...
        gtk_container_add ( GTK_CONTAINER ( pmCOMM ),
                            lookup_selection );
...
static void on_btnCOMMForward_clicked ( GtkButton * button,
                                        COMM_DATA * c )
{
        backend_nav_commentary_COMM ( c->modnum, 1 );
}

static void on_btnCOMMPrint_clicked ( GtkButton * button,
                                      COMM_DATA * c )
{
        html_print ( c->html );
}
                                   ...
                                   gint count )
{
        ...
        gtk_container_add ( GTK_CONTAINER ( s->notebookCOMM ),
                            frameCOMM );
...
void gui_setupCOMM ( SETTINGS * s )
{
        ...
        while ( tmp != NULL ) {
                ...
                c->modName = modname;
                c->modDescription =
                    backend_getmodDescriptionSWORD ( modname );
                c->modnum = count;
                c->searchstring = NULL;
                c->key = NULL;
                c->find_dialog = NULL;
                ...
                comm_list =
                    g_list_append ( comm_list, ( COMM_DATA * ) c );
                ++count;
                tmp = g_list_next ( tmp );
        }
