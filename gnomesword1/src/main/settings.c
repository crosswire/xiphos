...
gint settings_init(void)
{
	...
	if(have_old){
		...
		old_bm = g_new(char, strlen(old_dir) +
				strlen("/bookmarks") + 2);
		sprintf(old_bm, "%s/%s", old_dir,			
				"bookmarks");
		
		old_sb = g_new(char, strlen(old_dir) +
				strlen("/shortcutbar") + 2);
		sprintf(old_sb, "%s/%s", old_dir,			
				"shortcutbar");
			...
			if(have_old){
				...
				if (access(old_fav, F_OK) == 0) {
					...
					GList * glist = backend_load_sb_group(old_fav, group_name,
							 icon_size);
					backend_save_sb_group(glist, "Favorites.conf", group_name, 
						0, icon_size);
				}
				if (access(old_text, F_OK) == 0) {
					GList * glist = backend_load_sb_group(old_text, group_name,
							 icon_size);
					backend_save_sb_group(glist, "BibleText.conf", group_name, 
						0, icon_size);
				}
				if (access(old_comm, F_OK) == 0) {
					GList * glist = backend_load_sb_group(old_comm, group_name,
							 icon_size);
					backend_save_sb_group(glist, "Commentaries.conf", group_name, 
						0, icon_size);
				}
				if (access(old_dict, F_OK) == 0) {
					GList * glist = backend_load_sb_group(old_dict, group_name,
							 icon_size);
					backend_save_sb_group(glist, "Dictionaries.conf", group_name, 
						0, icon_size);
				}
				if (access(old_gbs, F_OK) == 0) {
					GList * glist = backend_load_sb_group(old_gbs, group_name,
							 icon_size);
					backend_save_sb_group(glist, "Books.conf", group_name, 
						0, icon_size);
				}
			}
		}
