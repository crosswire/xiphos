			...
			GtkHTMLParagraphStyle style, gpointer data)
{
	...
	g_warning 
	("Editor component toolbar: unknown paragraph style %d", style);
}
...
static void paragraph_style_menu_item_activated_cb (
				GtkWidget *widget, gpointer data)
{
	GtkHTMLParagraphStyle style;
	GtkHTML *html;
...
static void paragraph_style_option_menu_set_mode (
			GtkWidget *option_menu, gboolean format_html)
{
	GtkWidget *menu;
...
static GtkWidget *setup_paragraph_style_option_menu (GtkHTML *html)
{
	...
	gtk_signal_connect (GTK_OBJECT (html), 
			...
			GTK_SIGNAL_FUNC (paragraph_style_changed_cb), 
			option_menu);
...
static void set_font_size (GtkWidget *w, GSHTMLEditorControlData *cd)
{
	...
	if (!cd->block_font_style_change)
		...
		~GTK_HTML_FONT_STYLE_SIZE_MASK, style);
}
...
static GtkWidget *setup_font_size_option_menu (
				GSHTMLEditorControlData *cd)
{
	GtkWidget *option_menu;
	GtkWidget *menu;
			...
			gboolean by_user, GSHTMLEditorControlData *cd)
{
	...
	color = gdk_color
		&& gdk_color != &html_colorset_get_color (
...
static void set_color_combo (GtkHTML *html, GSHTMLEditorControlData *cd)
{
...
static GtkWidget *setup_color_combo (GSHTMLEditorControlData *cd)
{
	...
	color = html_colorset_get_color (
		cd->html->engine->settings->color_set, HTMLTextColor);
	if (GTK_WIDGET_REALIZED (cd->html))
	...
	cd->combo = color_combo_new (NULL, _("Automatic"), &color->color, 
		color_group_fetch ("toolbar_text", cd));
	...
	gtk_container_forall (GTK_CONTAINER (cd->combo), 
		...
		GTK_SIGNAL_FUNC (color_changed), cd);
