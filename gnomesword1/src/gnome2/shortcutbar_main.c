		   ...
		   GdkEvent * event, gint group_num, gint item_num)
{
	...
	gtk_menu_append(GTK_MENU(menu), menuitem);
	gtk_signal_connect(GTK_OBJECT(menuitem), "activate",
			   G_CALLBACK(on_about_item_activate),
...
void gui_setup_shortcut_bar(void)
{
	...
	*/	
	...
	gtk_signal_connect(GTK_OBJECT(shortcut_bar),
			   "shortcut_dragged",
			   G_CALLBACK(on_shortcut_dragged), NULL);
	gtk_signal_connect(GTK_OBJECT(shortcut_bar),
			   "shortcut_dropped",
			   G_CALLBACK(on_shortcut_dropped), NULL);
}
