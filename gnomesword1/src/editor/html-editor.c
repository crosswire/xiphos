			   ...
			   Bonobo_PersistFile pfile)
{
	...
	xml_set_value("GnomeSword", "studypad", "lastfile", e->filename);
		      ...
		      gpointer data)
{
	...
	if (interface == CORBA_OBJECT_NIL) {
		...
		case OP_LOAD_THROUGH_PERSIST_FILE:
			load_through_persist_file (data, g_strdup(fname), interface);
			break;
		case OP_SAVE_THROUGH_PERSIST_FILE:
			save_through_persist_file (data, g_strdup(fname), interface);
			break;
		default:
			      ...
			      gpointer data)
{
	...
	if(g_file_test(template, G_FILE_TEST_EXISTS)) {
		...
		load_through_persist_file (e, g_strdup(e->filename), e->persist_file_interface);
		g_free(e->filename);
		e->filename = NULL;
#ifdef DEBUG
		g_message(template);
...
gint ask_about_saving(EDITOR *e)
{
	...
	if(!e->studypad) {
		...
		buf3 = g_strdup_printf("<span weight=\"bold\" size=\"larger\">%s %s %s</span>",buf1,buf,buf2);
		info->label_top = buf3;
		info->label2 = N_("If you don't save, changes will be permanently lost.");
		info->save = TRUE;
		info->cancel = TRUE;
		info->no_save = TRUE;
...
container_create (const gchar * window_title, EDITOR * editor)
{
	...
	if(editor->studypad) {
		bonobo_ui_util_set_ui(component,
                                             INSTALL_PREFIX,
                                             gui_general_user_file ("/editor_studypad.xml", FALSE),
                                             "editor",
                                             &ev);
	} else {
		bonobo_ui_util_set_ui(component,
                                             INSTALL_PREFIX,
                                             gui_general_user_file ("/editor_note.xml", FALSE),
                                             "editor",
                                             &ev);
	} 
	CORBA_exception_free (&ev);
...
editor_create_new(const gchar * filename, const gchar * key, gint note)
{
	...
	if(note) {
		...
		load_through_persist_file (editor, g_strdup(filename), NULL);
...
load_file (EDITOR * e)
{
	...
	CORBA_exception_free (&ev);
	load_through_persist_file (e, e->filename, NULL);
	return FALSE;
}
#endif /* USE_GTKHTML38 */
