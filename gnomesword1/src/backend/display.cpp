 ...
 */ char EntryDisp::Display(SWModule & imodule)
{
        ...
        if (strbuf->len) {
                gtk_html_write(GTK_HTML(html), htmlstream, strbuf->str,
                               strbuf->len);
        }
...
char GtkHTMLEntryDisp::Display(SWModule & imodule)
{
        gchar
            ...
            *buf,
            *strkey,
            *font, *use_font, *use_font_size = NULL, *token, *utf8str;
        ...
        bool isGBS = false, isPerCom = false;

        if (!strcmp(imodule.Type(), "Generic Books"))
                isGBS = true;
        if (!strcmp
            ((gchar *) imodule.getConfigEntry("ModDrv"), "RawFiles"))
                isPerCom = true;
        ...
        if ((s->displaySearchResults)) {
                g_string_sprintf(str,
                               "<A HREF=\"version=%s passage=%s\"><FONT COLOR=\"%s\">[%s] %s </font></A>",
                                 ...
                                 (const char *) imodule.Name(), strkey);
        } else if (isGBS) {
                s->percomverse = (gchar *) imodule.KeyText();
                g_string_sprintf(str,
                              "<FONT COLOR=\"%s\">[%s]</font>",
                                 s->bible_verse_num_color, strkey);
        } else if (isPerCom) {
                s->percomverse = (gchar *) imodule.KeyText();
        } else {
                g_string_sprintf(str,
...
gchar *GtkHTMLEntryDisp::pick_font(SWModule & imodule)
{
        ...
        gsfont = NULL;
        lang = NULL;
        gsfont = (gchar *) imodule.getConfigEntry("Font"); 
        lang = (gchar *) imodule.getConfigEntry("Lang");        
             ...
             (key->Book() == curBook && key->Chapter() == curChapter
              && !imodule.Error()); imodule++) {
                ...
                if (newparagraph && s->versestyle) {
                        ...
                        utf8str =
                            e_utf8_from_gtk_string(gtkText, tmpBuf);
                        utf8len = strlen(utf8str);
                        if (utf8len) {
                ...
                if (s->versestyle) {
                        if ((strstr((const char *) imodule, "<BR>") ==
                             NULL)
                            && (strstr((const char *) imodule, "<!P>")
                                == NULL)) {
                                sprintf(tmpBuf, " %s", "</font><br>");
                        } else {
                                sprintf(tmpBuf, " %s", "</font>");
                        }
                        if (strstr((const char *) imodule, "<!P>") ==
                            NULL) {
                                newparagraph = false;
                        } else {
                                newparagraph = true;
                ...
                else if (strstr((const char *) imodule, "<!P>") == NULL)
                        sprintf(tmpBuf, " %s", "</font>");
                else
...
static void DisplayIndividualMod(SWModule * m,
                                 VerseKey * k,
                                 gchar * bgColor,
                                 gchar * textColor, SETTINGS * s)
{
        gchar * utf8str, buf[500], *use_font_size = NULL;
        ...
        sprintf(buf,
                ...
                k->Verse(),
                s->bible_verse_num_color,
                k->Verse(), use_font_size, textColor);
...
extern SWModule * interlinearMod0,      /* module for first interlinear window */
...
{
        bool utf = false, use_gtkhtml_font = false;
        ...
        gchar
            ...
            *font,
            *token,
            *lang = NULL, *use_font = NULL, *use_font_size = NULL;
