...
static void
on_cancel_clicked(GnomeDialog * gnomedialog, gint arg1, gpointer user_data)
{
    gtk_widget_destroy(gtk_widget_get_toplevel(GTK_WIDGET(gnomedialog)));
}
...
static GtkWidget *create_dlgSetup(void)
{
    ...
    checkbutton1 =
	gtk_check_button_new_with_label(_("Auto Save Personal comments "));
    gtk_widget_ref(checkbutton1);
    gtk_object_set_data_full(GTK_OBJECT(dlgSetup), "checkbutton1",
			     checkbutton1,
			     (GtkDestroyNotify) gtk_widget_unref);
    gtk_widget_show(checkbutton1);
    gtk_box_pack_start(GTK_BOX(hbox28), checkbutton1, FALSE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(hbox28), checkbutton5, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(hbox29), checkbutton3, FALSE, TRUE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(hbox29), checkbutton6, FALSE, FALSE, 0);
    ...
    gnome_druid_page_finish_set_title(GNOME_DRUID_PAGE_FINISH
				      (druidpagefinish1), gs_gnomesword);
    ...
    gtk_signal_connect(GTK_OBJECT(GNOME_DRUID_PAGE(druidpagefinish1)),
		       "finish", GTK_SIGNAL_FUNC(on_finish_clicked), NULL);
