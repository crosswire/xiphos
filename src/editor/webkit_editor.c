...
action_increase_indent_activate_cb(GtkWidget * widget, EDITOR * e)
{
	editor_execute_script
	    ("document.execCommand('Indent', false, false);", e);
...
{
	editor_execute_script
	    ("document.execCommand('Outdent', false, false);", e);
}
...
action_insert_image_activate_cb(GtkWidget * widget, EDITOR * e)
{
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
		...
		script =
		    g_strdup_printf
		    ("document.execCommand('insertImage', null, '%s');",
		     filename);
...
action_insert_outline_activate_cb(GtkWidget * widget, EDITOR * e)
{
	...
	script =
	    g_strdup
	    ("document.execCommand('insertHTML', null, \"<OL CLASS=L1><LI> </LI></OL> \");");
	editor_execute_script(script, e);
...
colorbutton1_color_set_cb(GtkColorButton * widget, EDITOR * e)
{
	...
	forecolor =
	    g_strdup_printf
	    ("document.execCommand('forecolor', null, '%s');", color_str);
...
colorbutton_highlight_color_set_cb(GtkColorButton * widget, EDITOR * e)
{
	...
	highligntcolor =
	    g_strdup_printf
	    ("document.execCommand('backColor', null, '%s');", color_str);
...
static void _save_file(EDITOR * e)
{
	...
	if (!e->filename
	    || (0 == g_strcmp0("Untitled document", e->filename))
...
static
gint _create_new(const gchar * filename, const gchar * key,
		 gint editor_type)
{
	EDITOR *editor;
//      GtkWidget *vbox = NULL;
		       ...
		       gint editor_type)
{
	...
	while (tmp != NULL) {
		...
		case NOTE_EDITOR:
			...
			e->key = g_strdup(key);
			gtk_widget_show(e->window);
			gdk_window_raise(gtk_widget_get_parent_window
					 (GTK_WIDGET(e->window)));
		...
		case BOOK_EDITOR:
			...
			gdk_window_raise(gtk_widget_get_parent_window
					 (GTK_WIDGET(e->window)));
			main_load_book_tree_in_editor(GTK_TREE_VIEW
						      (e->treeview),
						      e->module);
			editor_load_book(e);
