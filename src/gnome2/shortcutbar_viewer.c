						...
						gchar * verse_list)
{
	...
	g_string_printf(str,"<html><body bgcolor=\"%s\" text=\"%s\" link=\"%s\"><font color=\"%s\"><b>[%s]</b><br></font>",
	...
	while(tmp != NULL) {		
		...
		g_string_printf(str,"<a href=\"%s\"><font color=\"%s\"size=\"%s\">%s</font></a><br>",
					(const char *)tmp->data,
					colorkey,
					"+0",
					(const char *)tmp->data);
...
GtkWidget * gui_create_shortcutbar_viewer(GtkWidget *vboxVL) 
{
	...
	toolbarVL =
	    gtk_toolbar_new();
	gtk_toolbar_set_style (GTK_TOOLBAR (toolbarVL), GTK_TOOLBAR_ICONS);
...
/*	gtk_toolbar_set_button_relief(GTK_TOOLBAR(toolbarVL),
	...
	tmp_toolbar_icon = gtk_image_new_from_stock (
			"gtk-save", 
			gtk_toolbar_get_icon_size (GTK_TOOLBAR (toolbarVL)));
	sv->btn_save =
