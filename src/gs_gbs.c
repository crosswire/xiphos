			...
			unsigned long offset) 
{
	...
	while(backend_treekeyNextSiblingGBS(bookname,offset)){
		offset = backend_treekeyGetOffsetGBS(bookname);
		sprintf(buf,"%lu",offset);
...
static
void gui_addbooktoCTreeGBS(GtkWidget *ctree, gchar *bookName) 
{
...
{	
	...
	if(backend_displayRowGBS(bookname, offset)) {			     
	/** fill ctree node with children **/
		if((GTK_CTREE_ROW(treeNode)->children == NULL)&&(!GTK_CTREE_ROW(treeNode)->is_leaf)){
			gui_addNodeChildrenGBS(settings,treeNode,bookname, strtoul(offset,NULL,0));//load_book_tree(settings, treeNode, bookname,nodename,strtoul(offset,NULL,0));	
...
GtkWidget *createGBS_Pane(gchar *modName, SETTINGS *s,gint count, GBS_DATA *p_gbs) 
{
	...
	gui_addbooktoCTreeGBS(p_gbs->ctree, modName);
...
void gui_setupGBS(SETTINGS *s, GList *books)
{
	gint count = 0;
	GList *tmp = NULL;
	gchar *bookname;
