...
void AddSection(GtkCTree * ctree, SWConfig * config,
                const gchar * section, GtkCTreeNode * parent)
{
            ...
            config->Sections.end()) {
                for (eit = (*sit).second.begin();
                     eit != (*sit).second.end(); eit++) {
                        ...
                        if (is_leaf)
                                node =
                                    gtk_ctree_insert_node(ctree, parent,
                                                          node, text, 3,
                                                          pixmap3,
                                                          mask3, NULL,
                                                          NULL, is_leaf,
                        ...
                        gtk_ctree_node_set_row_data_full(ctree, node,
                                                         ...
                                                         (GtkDestroyNotify)
                                                         gtk_style_unref);
                        AddSection(ctree, config, (*eit).first.c_str(),
                                   node);
                }
        }
}
                ...
                if ((eit =
                     (*sit).second.begin()) != (*sit).second.end()) {
                        ...
                        *bmfiles.insert(bmfiles.begin(), conffile);
                        personal_node =
                            gtk_ctree_insert_node(ctree, NULL, NULL,
                                                  text, 3, pixmap1,
                                                  mask1, pixmap2, mask2,
                                                  FALSE, FALSE);
                        style = gtk_style_new();
                        ...
                        gtk_ctree_node_set_row_data_full(ctree,
                                                         ...
                                                         (GtkDestroyNotify)
                                                         gtk_style_unref);
                        AddSection(ctree, bookmarkInfo,
                                   (*eit).first.c_str(), personal_node);
                }
        }
        delete bookmarkInfo;
 ...
 * 
 ******************************************************************************/
void AddSectionToConf(SWConfig * config, string section,
                      GtkCTreeNode * node, int index)
{
        GtkCTreeNode *work;
        ConfigEntMap sit;
                ...
                for (work = GTK_CTREE_ROW(node)->children; work;
                     work = GTK_CTREE_ROW(work)->sibling) {
                        ...
                        sprintf(tmpbuf, "%s|%s|%s",
                                GTK_CELL_PIXTEXT(GTK_CTREE_ROW(work)->
                                                 row.cell[0])->text,
                                GTK_CELL_PIXTEXT(GTK_CTREE_ROW(work)->
                                                 row.cell[1])->text,
                                GTK_CELL_PIXTEXT(GTK_CTREE_ROW(work)->
 ...
 ******************************************************************************/
void createbookmarksBM(char *dir)
{
        char buf[255];
