...
GSMarkupFilterMgr::GSMarkupFilterMgr (char mark, char enc)
...
GSMarkupFilterMgr::~GSMarkupFilterMgr() {
        if (fromthml)
                delete (fromthml);
        if (fromgbf)
                delete (fromgbf);
 ...
 * RET: markup
 */
char GSMarkupFilterMgr::Markup(char mark) {
        if (mark && mark != markup) {
                markup = mark;
...
void GSMarkupFilterMgr::AddRenderFilters(SWModule *module, ConfigEntMap &section) {
...
void GSMarkupFilterMgr::CreateFilters(char markup) {
